<?php
	...
	if (isset($_POST["btn_upload"]) == "Upload")
	{
		...
		$file_tmp1 = $_FILES["fileaudio"]["tmp_name"];
		$file_name1 = $_FILES["fileaudio"]["name"];
		...
		//audio directory where actual image will be store
		$file_path1 = "audio/".$file_name1;	

	/*---------------- php textbox validation checking ------------------*/
	if($image_name == "")
	{
		$error = "Please enter file.";
	}

	/*-------- now insertion of image section has start -------------*/
	else
	{
		if(file_exists($file_path) or file_exists($file_path1))
		{
			...
			else
			{
				...
				mysqli_query($result,"INSERT INTO image_table(img_name,img_path, img_audio)
				VALUES('$image_name','$file_path', '$file_path1')") or die ("image not inserted". mysqli_error($result));
